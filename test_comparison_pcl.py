...
def compression_pcl(quant, pc_ori, cur_name):
    path_tmc3 = '/home/install/exp_pcc/pcl_exp/pcl_exp/build/'
    path_in_pc = './test_comparison/temp/pcl_in_' + cur_name + '.ply'
    path_bin = './test_comparison/temp/pcl_' + cur_name + '.bin'
    path_out_pc = './test_comparison/temp/pcl_out_' + cur_name + '.ply'

    pc_quant = (pc_ori * quant).astype(np.int32)
    save_point_cloud_to_file(path_in_pc, pc_quant)

    command = path_tmc3 + 'pcl_pcc ' + \
        ' --input_ply_path=' + path_in_pc + \
        ' --bin_path=' + path_bin + \
        ' --ply_path=' + path_out_pc
    compression = os.popen(command)
    time.sleep(1)
...
def test_seq_pcl(args, pcl_quant):
    ...
    for step, data in tqdm(enumerate(test_loader),
        ...
        ''' compression '''
        pc_rec, path_bitstream = compression_pcl(pcl_quant,
                                                  pc_ori,
                                                  cur_name)
...
if __name__ == '__main__':
    ...
    # list_pcl_quant = [1000, 500, 250, 100, 50, 20, 15, 10, 5]
    list_pcl_quant = [0.7]
    for i_quant in list_pcl_quant:
        test_seq_pcl(args, i_quant)
