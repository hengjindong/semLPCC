...
def parse_args():
    ...
    parser.add_argument('--compression_method', type=str, default='mpeg')
    parser.add_argument('--mpeg_quant', type=int, default=255)
    parser.add_argument('--draco_qp', type=int, default=15)
...
def compression_draco(qp, pc_ori, cur_name):
    path_draco = '/home/install/exp_pcc/draco-master/build/'
    # path_in_pc = './test_comparison/temp/draco_in_' + cur_name + '.ply'
    path_in_pc = './test_comparison/temp/draco_in_' + 'aaa' + '.ply'
    # path_drc = './test_comparison/temp/draco' + cur_name + '.drc'
    path_drc = './test_comparison/temp/draco' + 'aaa' + '.drc'
    # path_out_pc = './test_comparison/temp/draco_out_' + cur_name + '.ply'
    path_out_pc = './test_comparison/temp/draco_out_' + 'aaa' + '.ply'

    ''' encode  '''
    save_point_cloud_to_file(path_in_pc, pc_ori)
    command_encoder = path_draco + 'draco_encoder -point_cloud'  + \
        ' -i ' + path_in_pc + \
        ' -o ' + path_drc + \
        ' -cl 10' + ' -qp ' + str(qp)
    ...
    ''' decode  '''
    command_decoder = path_draco + 'draco_decoder' + \
        ' -i ' + path_drc + \
        ' -o ' + path_out_pc
...
def test_seq_draco(args, draco_qp):
    ...
    for step, data in tqdm(enumerate(test_loader),
        ...
        ''' compression '''
        pc_rec, path_bitstream = compression_draco(draco_qp,
                                                  pc_ori,
                                                  cur_name)
...
if __name__ == '__main__':
    ...
    list_draco_qp = [16, 15, 14, 13, 12, 11, 10, 9]
    for i_qp in list_draco_qp:
        test_seq_draco(args, i_qp)
