...
def parse_args():
    ...
    parser.add_argument('--loss_bit_range', type=int, default=10)
...
def eval_depth_rate(depth):
    save_png_path = './temp/temp_Intra_R_GS_' + \
...
if __name__ == '__main__':
    ...
    for step, data in tqdm(enumerate(test_loader),
        ...
        depth_scene = depth_scene + depth_object
        ...
        pc_total_ori = np.concatenate((pc_ground,
                                       pc_scene), axis=0)
        ...
        for i_bit in range(args.loss_bit_range):
            ...
            pc_total_recon = np.concatenate((pc_ground,
                                             pc_scene_quant), axis=0)

            rmse = eval_pc_rmse(pc_total_recon, pc_total_ori)

            all_lossly_bpp[i_bit][step] = bpp_scene
            all_lossly_fsize[i_bit][step] = fsize_bit_scene
    ...
    np.savetxt('./exp_intra/Intra_R_GS_'+args.seq_name+'.txt', save_txt, '%0.8f')
