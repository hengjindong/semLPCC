...
if __name__ == '__main__':
    ...
    for step, data in tqdm(enumerate(test_loader),
        ...
        o3d_recon = o3d_source.transform(result_icp.transformation)

        ''' border res cal '''
        for i_bit in range(args.loss_bit_range):
            depth_res, pc_recon_add_res, pc_target = \
                pc_border_res(o3d_recon, o3d_target, args.border_num, i_bit)

            bpp, fsize_bit = eval_depth_rate(depth_res)

            rmse = eval_pc_rmse(pc_recon_add_res, pc_target)
    ...
    np.savetxt('./exp_inter/Inter_ME_B_'+str(args.border_num)+'_'+args.seq_name+'.txt', save_txt, '%0.8f')
