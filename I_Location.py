...
def eval_depth_rate(depth):
    save_png_path = './temp/location_depth_' + \
...
def location_eval(o3d_scene_target, o3d_scene_source):
    ...
    return result_icp.transformation, cost_time
...
if __name__ == '__main__':
    ...
    all_lossly_time = np.zeros((args.loss_bit_range, len(test_loader)))

    all_lossly_icp_error = np.zeros((args.loss_bit_range, len(test_loader)))
    ...
    for step, data in tqdm(enumerate(test_loader),
        ...
        ori_trans, ori_time = location_eval(o3d_scene_target, o3d_scene_source)

        o3d_scene_recon = o3d_scene_source.transform(ori_trans)
        ...
        for i_bit in range(args.loss_bit_range):
            ...
            ''' new transfrom '''
            o3d_scene_target = np_pc_to_o3d_pc(pc_scene_recon_add_res)
            this_trans, this_time = location_eval(o3d_scene_target, o3d_scene_source)
            icp_error = this_trans - ori_trans
            print(this_trans)
            print(this_time)
            print(icp_error.mean())
            pdb.set_trace()
            ...
            all_lossly_time[i_bit][step] = this_time
            all_lossly_icp_error[i_bit][step] = icp_error.mean()
